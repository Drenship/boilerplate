...
export async function putUser(updatedData: Partial<typeof User>) {
  try {
    // Authentifie l'utilisateur via la session
    const session = await auth();
    if (!session) {
...
export async function createAndSendEmailVerification() {
  try {
    ...
    const verificationTokenExpires = new Date(Date.now() + 24 * 60 * 60 * 1000); // Expire dans 24 heures
...
export async function getLoginSessionHistory() {
  try {
    const session = await auth();
    if (!session || !session._id) {
      return { failure: "not authenticated" };
...
export async function searchUsers({
  ...
  try {
    ...
    const users = await User.find(queryObj)
      .select("name email emailVerification.isVerified image isAdmin createdAt") // Exclure les champs sensibles
      .limit(limit)
      .skip(skip)
      .lean();
