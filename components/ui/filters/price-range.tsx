...
import { Slider } from "@/components/ui/dual-range-slider";
import { useId, useMemo } from "react";

type DataRange = { id: number | string; price: number };
...
function PriceRange({ items = mock_items }: { items: DataRange[] }) {
  const id = useId();
  ...
  const countItemsInRange = (min: number, max: number) => {
    return items && items.length > 0
      ? items.filter((item) => item.price >= min && item.price <= max).length
      : 0;
  };
